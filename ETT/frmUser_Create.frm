...
Private Sub CreateNewUser() 'For Saving the Record in Database.
    With RstCrtUsr
        .AddNew
            If txtUserName.Text = "" Then .Fields(0).Value = "Unknown"
            If txtUserName.Text <> "" Then .Fields(0).Value = txtUserName.Text
            
            .Fields(1).Value = txtUserID.Text: .Fields(2).Value = txtPwd.Text
            .Fields(3).Value = Opt_Stat: .Fields(4).Value = Opt_Priv
        .Update
...
Private Sub Populate_User_Existance()
    With RstCrtUsr
        .Close: .Open "SELECT * FROM tblLogIn WHERE User_ID='" & txtUserID.Text & "'"
...
Private Sub CmdCreate_Click()
    ...
    Call Ctrl_ET.Populate_Text_Clear(frmUser_Create) 'Initiate TextBoxes(Clear Textboxes).
...
Private Sub CmdExit_Click()
    ...
    ElseIf Opt_End = vbNo Then
        Call Ctrl_ET.Populate_Text_Clear(frmUser_Create) 'Call for Clearing the Text Boxes.
...
Private Sub Form_Load()
    ...
    RstCrtUsr.Open "SELECT * FROM tblLogin", DB_Conect, adOpenStatic, adLockOptimistic
End Sub
...
Private Sub txtUserName_KeyPress(KeyAscii As Integer)
    Call Ctrl_ET.Populate_Alpha_Char(KeyAscii, frmUser_Create, txtUserName) 'Call For Alpha Character(Capital).
    If KeyAscii = 13 Then
